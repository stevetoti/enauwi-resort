...
export async function GET() {
  try {
    ...
    return new Promise<NextResponse>((resolve, reject) => {
      ...
      doc.on('error', () => {
        reject(new NextResponse(JSON.stringify({ error: 'PDF generation failed' }), { status: 500 }));
      });
    });
...
// POST endpoint to generate and save the PDF file
export async function POST() {
  try {
    // Get the PDF by calling the GET handler internally
